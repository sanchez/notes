...
- Timesharing (multitasking) is logical extension in which CPU switches jobs so frequently that users can interact with each job while it is running, creating interactive computing
    ...
    - If several jobs ready to run at the same time &rarr; CPU scheduling
...
![Process in Memory](comp3301/Mem.png)
...
- As a process executes, it changes ==state==
    ...
    - **Terminated:** The process has finished execution
      ![Diagram of Process State](comp3301/state.png)
...
- Consider having multiple program counters per process
    - Multiple location can execute at once
        - Multiple threads of control &rarr; ==threads==
...
- Short-term scheduler is invoked very frequently (milliseconds) &rarr; (must be fast)
- Long-term scheduler is invoked very infrequently (seconds, minutes) &rarr; (may be slow)
...
- **Medium-term scheduler** can be added if degree of multiple programming needs to decrease
    - Remove process from memory, store on disk, bring back in from disk to continue execution &rarr; ==swapping==
...
- Context-switch time is overhead; the system does no useful work while switching
    - The more complex the OS and the PCB &rarr; longer the context switch
...
![Communications Models](comp3301/communications.png)
...
![Multithreaded Server Architecture](comp3301/thread.png)
...
![Single and Multithreaded Processes](comp3301/single.png)
...
```c
dispatch_queue_t queue = dispatch_get_global\_queue(DISPATCH\_QUEUE_PRIORITY\_DEFAULT, 0);
...
- CPU burst distribution is of main concern
  ![CPU Burst](comp3301/cpuburst.png)
...
- Can be done by using the length of previous CPU bursts, using exponential averaging
    1. $t_n = \text{actual length of } n_{th}\text{ CPU burst}$
    2. *\mathcal{t}{n+1}* = predicted value for the next CPU burst
    3. $\mathcal{t}_{n+1}$ = predicted value for the next CPU burst
    4. $\alpha, 0\leq\alpha\leq1$
    5. Define:
...
![Preemptive SJF Gantt Chart](comp3301/SRTF.png)
...
![Priority Scheduling Example](comp3301/priority.png)
...
- If there are *n* processes in the ready queue and the time quantum is $q$, then each process gets $\frac{1}{n}$ of the CPU time in chunks of at most $q$ time units at once. No process waits more than $(1-n)q$ time units.
- Timer interrupts every quantum to schedule next process
- Performance
    - *q* large &rarr; FIFO
    - *q* small &rarr; $q$ must be large with respect to context switch, otherwise overhead is too high
...
- Can be analysed to detect deadlock
  ![Resource-Allocation Graph](comp3301/resource.png)
...
- System is in **safe state** if there exists a sequence $P_1, P_2, \ldots, P_n$ of ALL processes in the systems such that for each $P_i$, the resources that $P_i$ can still request can be satisfied by currently available resources + resources held by all the $P_j$, with $j<i$
- That is:
...
![Multistep Processing of a User Program](comp3301/multistep.png)
...
- First fit analysis reveals that given *N* blocks allocated, 0.5 *N* blocks lost to fragmentation
    - 1/3 may b unusable &rarr; ==50-percent rule==
...
![Paging Hardware](comp3301/pageHardware.png)
...
![Paging Hardware with TLB](comp3301/tlb.png)
...
- Memory structures for paging can get huge using straight-forware methods
    ...
    - If each entry is 4 bytes &rarr; 4MB of physical address space / memory for page table alone
...
- We then page the page table
  ![Two-Level Page-Table Scheme](comp3301/2page.png)
...
- Consider ability to execute partially-loaded program
    ...
    - Each program takes less memory while running &rarr; more programs run at the same time
        - Increased CPU utilization and throughput with no increase in response time or turnaround time
    - Less I/O needed to load or swap programs into memory &rarr; each user program runs faster
...
5. Restart the instruction that caused the page fault
  ![Steps in Handling a Page Fault](comp3301/fault.png)

### Aspects of Deman Paging

- Extreme case — start process with *no* pages in memory
    - OS sets instruction pointer to first instruction of process, non-memory-resident &rarr; page fault
    - And for every other process pages on first access
    - **Pure demand paging**
- Actually, a given instruction could access multiple pages &rarr; multiple page faults
...
- Policy if D > m, then suspend or swap out one of the processes
  ![Working-Set Model](comp3301/workingSet.png)
...
- Less capacity
- But much faster
- Busses can be too slow &rarr; connect directly to PCI for example
...
- Storage Array has controller(s), provides features to attached host(s)
    ...
    - Shared storage &rarr; more efficiency
...
- Multiple hosts attached to multiple storage arrays - flexible
  ![Storage Area Network](comp3301/san.png)
...
![A Typical File-system Organization](comp3301/file.png)
...
![In-Memory File System Structures](comp3301/structure.png)
...
- Declare access type at creation &rarr; select either contiguous or linked
...
|                    | block                    | disk                |
| access method      | sequential               | modern              |
|                    | random                   | CD-ROM              |
...
- For example, Android implements
    - Component-level power management
        ...
        - System bus &rarr; I/O subsystem &rarr; \{flash, USB storage\}
